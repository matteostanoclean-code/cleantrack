    ...
    <Shell>
      <div className="flex min-h-[calc(100vh-4rem)] flex-col justify-center">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">Mitarbeiterstunden, Lohnkosten und Überlastung prüfen.</p>
        </div>
...
function StatCard({ title, value, caption, tone = "blue" }: { title: string; value: string | number; caption: string; tone?: "blue" | "green" | "red" | "amber" }) {
  const toneClass = tone === "red" ? "text-red-100" : tone === "green" ? "text-emerald-100" : tone === "amber" ? "text-amber-100" : "text-white";
  return (
...
export default function AdminCapacityPage() {
  ...
  if (authLoading) return <Shell><p className="p-4 text-sm text-slate-400">Prüfe Login…</p></Shell>;
      ...
      <div className="space-y-4 pb-24">
        <header className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">Monatsplanung · {range.label}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-xs font-bold uppercase tracking-wide text-slate-500">Monat</span>
            <input value={month} onChange={(event) => setMonth(event.target.value)} type="month" className={inputClass} />
          </label>
          <label className="block">
            <span className="text-xs font-bold uppercase tracking-wide text-slate-500">Suche</span>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-slate-400">Überlastung, offene Stempeluhren und unbesetzte Einsätze.</p>
            </div>
            <button onClick={exportCsv} className="rounded-2xl bg-blue-600 px-4 py-3 text-sm font-black text-white shadow-glow">CSV</button>
          </div>
          <div className="mt-3 grid gap-2">
            {totals.overloads ? <p className="rounded-2xl border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-100">{totals.overloads} Mitarbeiter sind über dem Monatslimit geplant.</p> : <p className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-100">Keine Überlastung im sichtbaren Monat.</p>}
            ...
            {rows.some((row) => row.openClock) ? <p className="rounded-2xl border border-blue-500/30 bg-blue-500/10 p-3 text-sm text-blue-100">Mindestens eine Stempeluhr ist noch offen.</p> : null}
          </div>
        </section>
        ...
        <section className="space-y-3">
          ...
          {rows.map((row) => {
            ...
            return (
              <div key={row.employee.id || row.name} className={`rounded-3xl border p-4 ${overload ? "border-red-500/30 bg-red-500/10" : "border-slate-800 bg-slate-900/80"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black text-white">{row.name}</p>
                    <p className="mt-1 text-xs text-slate-400">{row.assigned.length} Einsätze · {row.vacationDays} Urlaub/Frei-Tage</p>
                  </div>
                  <span className={`rounded-full px-3 py-1 text-xs font-black ${overload ? "bg-red-500/20 text-red-100" : "bg-blue-500/15 text-blue-100"}`}>{percent(row.utilization)}</span>
                </div>
                <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-800">
                  <div className={`h-full rounded-full ${overload ? "bg-red-500" : "bg-blue-500"}`} style={{ width: `${Math.min(100, progress)}%` }} />
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                  <div className="rounded-2xl bg-slate-950 p-3"><p className="text-xs text-slate-500">Soll</p><p className="font-black">{row.limitMinutes ? hours(row.limitMinutes) : "nicht gesetzt"}</p></div>
                  <div className="rounded-2xl bg-slate-950 p-3"><p className="text-xs text-slate-500">Plan</p><p className="font-black">{hours(row.planMinutes)}</p></div>
                  <div className="rounded-2xl bg-slate-950 p-3"><p className="text-xs text-slate-500">Ist</p><p className="font-black">{hours(row.actualMinutes)}</p></div>
                  <div className="rounded-2xl bg-slate-950 p-3"><p className="text-xs text-slate-500">Lohnkosten</p><p className="font-black">{money(row.planCost)}</p></div>
                </div>
                <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-black uppercase tracking-wide">
                  <span className="rounded-full bg-slate-800 px-3 py-1 text-slate-300">{row.hourlyRate ? `${row.hourlyRate.toLocaleString("de-DE")} €/h` : "kein Stundensatz"}</span>
                  <span className="rounded-full bg-slate-800 px-3 py-1 text-slate-300">Faktor {row.factor.toLocaleString("de-DE")}</span>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400">Diese Einsätze sollten in der Planungszentrale verteilt werden.</p>
            </div>
            <Link href="/mitarbeiter/admin/planung" className="text-xs font-black text-blue-200">Planen</Link>
          </div>
              ...
              <div key={task.id} className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-400">{dateText(task.task_date)} · {clean(task.start_time) || "—"} - {clean(task.end_time) || "—"}</p>
                    <p className="mt-1 text-xs text-slate-500">{taskPlace(task)}</p>
                  </div>
                  <span className="rounded-full bg-blue-500/15 px-3 py-1 text-xs font-black text-blue-100">{hours(taskMinutes(task))}</span>
                </div>
              </div>
            )) : <p className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-100">Alle sichtbaren Einsätze sind zugewiesen.</p>}
          </div>
        </section>

        <section className="grid gap-2">
          <Link href="/mitarbeiter/admin" className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-center font-black text-blue-100">Zurück zum Admin-Dashboard</Link>
          <Link href="/mitarbeiter" className="rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-center font-black text-slate-200">Zur Mitarbeiter-App</Link>
        </section>
      </div>
    </Shell>
  );
