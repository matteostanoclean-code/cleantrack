...
function AdminPage() {
  ...
  /**
   * Reiter aus der Adresse übernehmen, etwa ?tab=chat aus der Seitenleiste.
   * Als Effekt und nicht als Startwert, weil ein Wechsel innerhalb derselben
   * Seite die Komponente nicht neu aufbaut — der Startwert liefe dann nie.
   */
  useEffect(() => {
    const gewuenscht = suchparameter?.get("tab") || "";
    const erlaubt = ["time", "absence", "material", "quality", "service", "chat", "notifications"];
    if (erlaubt.includes(gewuenscht)) setTab(gewuenscht as Tab);
  }, [suchparameter]);
