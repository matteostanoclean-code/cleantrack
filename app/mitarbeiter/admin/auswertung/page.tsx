    ...
    <Shell>
      <div className="flex min-h-[calc(100vh-4rem)] flex-col justify-center">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">Bitte als Admin anmelden.</p>
        </div>
...
function Stat({ title, value, caption, tone = "text-white" }: { title: string; value: string | number; caption: string; tone?: string }) {
  return (
...
export default function AdminAuswertungPage() {
  ...
  if (authLoading) {
    return <Shell><div className="grid min-h-[70vh] place-items-center text-center text-slate-400">Lade Login…</div></Shell>;
  }
    ...
    <Shell>
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-wide text-blue-300">Admin</p>
            <h1 className="text-3xl font-black">Monatsauswertung</h1>
            <p className="mt-1 text-sm text-slate-400">Stunden, Einsätze und GPS-Kontrolle.</p>
          </div>
          <Link href="/mitarbeiter" className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-black text-blue-100">App</Link>
        </div>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4">
          <div className="grid gap-3">
            ...
            <button onClick={() => load()} disabled={loading} className="rounded-2xl bg-blue-600 py-4 font-black text-white shadow-glow disabled:opacity-60">{loading ? "Lade…" : "Auswertung laden"}</button>
          </div>
        </section>
        ...
        {data ? (
          <>
            <section className="grid grid-cols-2 gap-3">
              <Stat title="Ist-Stunden" value={hours(data.summary.totalMinutes)} caption="aus Stempelzeiten" tone="text-blue-100" />
              <Stat title="Plan-Stunden" value={hours(data.summary.plannedMinutes)} caption="aus Einsätzen" />
              <Stat title="Einsätze" value={data.summary.taskCount} caption={`${data.summary.doneTasks} erledigt · ${data.summary.openTasks} offen`} tone="text-emerald-100" />
              <Stat title="Kontrolle" value={data.summary.distanceWarnings} caption={`${data.summary.openSessions} offene Buchungen`} tone={data.summary.distanceWarnings || data.summary.openSessions ? "text-red-100" : "text-slate-100"} />
            </section>
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Für Lohnabrechnung und Kontrolle</p>
                </div>
                <button onClick={exportEmployeeCsv} className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-black text-white">CSV</button>
              </div>
              <div className="space-y-2">
                {data.summary.byEmployee.length ? data.summary.byEmployee.map((item) => (
                  <article key={item.employeeName} className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate font-black text-white">{item.employeeName}</p>
                        <p className="text-xs text-slate-500">{item.days} Tage · {item.pauses} Pausen</p>
                      </div>
                      <p className="text-lg font-black text-blue-100">{hours(item.minutes)}</p>
                    </div>
                    {item.openSessions || item.distanceWarnings ? <p className="mt-2 text-xs text-red-200">Prüfen: {item.openSessions} offene Buchung(en), {item.distanceWarnings} GPS-Warnung(en)</p> : null}
                  </article>
                )) : <p className="text-sm text-slate-500">Keine Stempelzeiten im gewählten Monat.</p>}
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4">
              ...
              <div className="space-y-2">
                {data.summary.bySite.slice(0, 15).map((item) => (
                  <div key={item.siteName} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-950 px-3 py-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-slate-100">{item.siteName}</p>
                      <p className="text-xs text-slate-500">{item.entries} Buchungen</p>
                    </div>
                    <p className="font-black text-blue-100">{hours(item.minutes)}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Letzte Buchungen im Monat</p>
                </div>
                <button onClick={exportEntriesCsv} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-black text-blue-100">Export</button>
              </div>
              <div className="space-y-2">
                {latestEntries.length ? latestEntries.map((entry) => (
                  <article key={entry.id} className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-black text-white">{actionLabel(entry.action)}</p>
                        <p className="truncate text-xs text-slate-400">{entry.employee_name || "Ohne Mitarbeiter"} · {entry.work_site_name || "Ohne Objekt"}</p>
                      </div>
                      <p className="text-xs font-bold text-blue-100">{formatDateTime(entry.created_at)}</p>
                    </div>
                    {typeof entry.distance_m === "number" ? <p className="mt-2 text-xs text-slate-500">GPS: {entry.distance_m} m · Radius {entry.allowed_radius_m || "—"} m</p> : null}
                  </article>
                )) : <p className="text-sm text-slate-500">Keine Buchungen gefunden.</p>}
              </div>
            </section>
          </>
        ) : null}
