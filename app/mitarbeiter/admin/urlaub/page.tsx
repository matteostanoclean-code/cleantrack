                ...
                <div key={absence.id} className="rounded-3xl border border-paper-300 bg-paper-100 p-4">
                  ...
                  <div className="mt-3 grid gap-2">
                    {!isClosed ? (
                      ...
                      <button disabled={saving} onClick={() => patchAbsence({ id: absence.id, action: "unassign_conflicts" }, "Einsatz-Konflikte wurden freigegeben.")} className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm font-black text-amber-100 disabled:opacity-50">Konflikte jetzt freigeben</button>
                    ) : null}
                  </div>
                </div>
              );
